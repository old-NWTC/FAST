...
ENDIF
   ...
   ! NOTE: y_StrD%AllOuts(Time) is scaled by OnePlusEps to ensure that the controller
...
!write(71,'(4(f20.10,2x))'), AllOuts(Time), LastTime, AllOuts(Time)*OnePlusEps  - LastTime, DTCntrl
   ...
   avrSWAP( 2) = y_StrD%AllOuts(     Time)                      ! Current time (sec)
   avrSWAP( 3) = y_StrD%AllOuts(     Time) - LastTime           ! Communication interval (sec)
   avrSWAP( 4) = y_StrD%AllOuts(PtchPMzc1)*D2R                  ! Blade 1 pitch angle (rad)
   ...
   avrSWAP(14) = y_StrD%AllOuts(   RotPwr)*1000.                ! Measured shaft power (W)
   avrSWAP(15) = y_StrD%AllOuts(   GenPwr)*1000.                ! Measured electrical power output (W)
   ...
   avrSWAP(20) = y_StrD%AllOuts(  HSShftV)*RPM2RPS              ! Measured generator speed (rad/s)
   avrSWAP(21) = y_StrD%AllOuts(LSSTipVxa)*RPM2RPS              ! Measured rotor speed (rad/s)
   avrSWAP(22) = GenTrq_Dem                              ! Demanded generator torque (Nm)
   avrSWAP(23) = y_StrD%AllOuts(    GenTq)*1000.                ! Measured generator torque (Nm)
   avrSWAP(24) = y_StrD%AllOuts(NacYawErr)*D2R                  ! Measured yaw error (rad)
   IF ( N == 0 )  THEN  ! Torque-speed table look-up not selected
   ...
   ENDIF
   avrSWAP(27) = y_StrD%AllOuts( HorWindV)                      ! Hub wind speed (m/s)
   ...
   avrSWAP(37) = y_StrD%AllOuts(   YawPzn)*D2R &
               + y_StrD%AllOuts(YawBrRDzt)*D2R &
               + y_StrD%AllOuts( PtfmRDzi)*D2R - NacYaw_North   ! Nacelle yaw angle from North (rad)
...
! Record 52 is reserved for future use                   ! DLL interface version number (-)
   avrSWAP(53) = y_StrD%AllOuts(YawBrTAxp)                      ! Tower top fore-aft     acceleration (m/s^2)
   avrSWAP(54) = y_StrD%AllOuts(YawBrTAyp)                      ! Tower top side-to-side acceleration (m/s^2)
...
! Records 57-59 are reserved
   avrSWAP(60) = y_StrD%AllOuts(LSSTipPxa)*D2R                  ! Rotor azimuth angle (rad)
...
! Records 66-68 are reserved
   avrSWAP(69) = y_StrD%AllOuts( RootMxc1)*1000.                ! Blade 1 root in-plane bending moment (Nm)
   avrSWAP(70) = y_StrD%AllOuts( RootMxc2)*1000.                ! Blade 2 root in-plane bending moment (Nm)
   avrSWAP(71) = y_StrD%AllOuts( RootMxc3)*1000.                ! Blade 3 root in-plane bending moment (Nm)
! Record 72 is the generator start-up resistance returned by the DLL [see the lines of code after the CALL to DISCON()]
   avrSWAP(73) = y_StrD%AllOuts(LSSTipMya)*1000.                ! Rotating hub My (GL co-ords) (Nm)
   avrSWAP(74) = y_StrD%AllOuts(LSSTipMza)*1000.                ! Rotating hub Mz (GL co-ords) (Nm)
   avrSWAP(75) = y_StrD%AllOuts(LSSTipMys)*1000.                ! Fixed hub My (GL co-ords) (Nm)
   avrSWAP(76) = y_StrD%AllOuts(LSSTipMzs)*1000.                ! Fixed hub Mz (GL co-ords) (Nm)
   avrSWAP(77) = y_StrD%AllOuts( YawBrMyn)*1000.                ! Yaw bearing My (GL co-ords) (Nm)
   avrSWAP(78) = y_StrD%AllOuts( YawBrMzn)*1000.                ! Yaw bearing Mz (GL co-ords) (Nm)
...
! Records 80-81 are demands returned by the DLL [see the lines of code after the CALL to DISCON()]
   avrSWAP(82) = y_StrD%AllOuts(NcIMURAxs)*D2R                  ! Nacelle roll    acceleration (rad/s^2) -- this is in the shaft (tilted) coordinate system, instead of the nacelle (nontilted) coordinate system
   avrSWAP(83) = y_StrD%AllOuts(NcIMURAys)*D2R                  ! Nacelle nodding acceleration (rad/s^2)
   avrSWAP(84) = y_StrD%AllOuts(NcIMURAzs)*D2R                  ! Nacelle yaw     acceleration (rad/s^2) -- this is in the shaft (tilted) coordinate system, instead of the nacelle (nontilted) coordinate system
