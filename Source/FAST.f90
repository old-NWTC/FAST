...
!ELSE                    ! Do    use MOD when computing y%AllOuts(LSSTipPxa) and y%AllOuts(LSSGagPxa)
   y%AllOuts(LSSTipPxa) = MOD( ( x%QT (DOF_GeAz) + x%QT  (DOF_DrTr) + p%AzimB1Up)*R2D  + 90.0, 360.0 )
   y%AllOuts(LSSGagPxa) = MOD( ( x%QT (DOF_GeAz)                    + p%AzimB1Up)*R2D  + 90.0, 360.0 )
!ENDIF
...
IF ( y%AllOuts(LSShftFxa) /= 0.0 )  THEN ! .TRUE. if the denominator in the following equations is not zero.
   ...
   !ELSE                    ! Do    use MOD when computing y%AllOuts(CThrstAzm)
      y%AllOuts(CThrstAzm) = MOD( ( ATAN2( -CThrstzs, -CThrstys ) + p%AzimB1Up )*R2D + 360.0 + 90.0, 360.0 )
   !ENDIF
