...
!bjj: should we first check that NumOuts is a valid number (or at least initialize it somewhere)?   
   ...
   IF ( Echo )  THEN
      ...
      WRITE (UnEc,OutPFmt)  0, p_StrD%OutParam(0)%Name, p_StrD%OutParam(0)%Units
   ENDIF
   ...
   DO I = 1,p_StrD%NumOuts
      ...
      IF ( Echo )  WRITE (UnEc,OutPFmt)  I, p_StrD%OutParam(I)%Name, p_StrD%OutParam(I)%Units
...
!=======================================================================
SUBROUTINE GetBladeInputs ( BldFile, p, InputFileData, ErrStat, ErrMsg )
   ...
   DO K = 1,p%NumBl   
      ...
      IF ( ReadFile ) THEN
         ...
         IF ( Echo )  WRITE (UnEc,'(//,A,/)')  'Blade '//TRIM( Num2LStr( K ) )//' input data from file "'//TRIM( BldFile(K) )//'":'
         
         CALL ReadBladeFile( BldFile(K), p, InputFileData%InpBl(K), ReadAdmVals, ErrStat2, ErrMsg2 )
...
!=======================================================================
SUBROUTINE GetPrimary( InputFileData, p, ErrStat, ErrMsg )
