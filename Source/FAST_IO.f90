...
!   CHARACTER(28)                :: OutPFmt                                         ! Format parameter printing output list to echo file
...
! This code was generated by Write_ChckOutLst.m at 30-Jan-2012 12:30:08.
   CHARACTER(OutStrLenM),PARAMETER  :: ValidParamAry(1110) =  (/ &                  ! This lists the names of the allowed parameters, which must be sorted alphabetically
   ...
   CHARACTER(OutStrLen),PARAMETER  :: ParamUnitsAry(1110) =  (/ &                         ! This lists the units corresponding to the allowed parameters
...
!   OutPFmt = '( I4, 3X,A '//TRIM(Num2LStr(OutStrLen))//',1 X, A'//TRIM(Num2LStr(OutStrLen))//' )'
   ...
   DO I = 1,p_StrD%NumOuts
      ...
      Indx =  IndexCharAry( OutListTmp(1:OutStrLenM), ValidParamAry )
...
!  -------------- TOWER (CONT) -------------------------------------------------
         ...
         IF ( UnEc > 0 )  WRITE (UnEc,'(A)')  &
            '  ---------------------- TOWER (CONT) --------------------------------------------                    '


   ! TwrLdMod - Tower loading model switch.

         CALL ReadVar ( UnIn, PtfmFile, p%TwrLdMod, 'TwrLdMod', 'Tower loading model switch' )

         IF ( ( p%TwrLdMod /= 0 ) .AND. ( p%TwrLdMod /= 1 ) .AND. ( p%TwrLdMod /= 2 ) )  THEN
            CALL ProgAbort ( ' TwrLdMod must be 0, 1, or 2.' )
         END IF


         IF ( p%TwrLdMod == 1 )  THEN ! .TRUE if we will be using the built-in Morison's equation.


   ! TwrDiam - Tower diameter in Morison's equation.

            CALL ReadVar ( UnIn, PtfmFile, InputFileData%TwrDiam, 'TwrDiam', 'Tower diameter in Morison''s equation' )

            IF ( InputFileData%TwrDiam < 0.0 )  CALL ProgAbort ( ' TwrDiam must not be negative.' )


   ! TwrCA - Normalized hydrodynamic added mass coefficient in Morison's equation.

            CALL ReadVar ( UnIn, PtfmFile, InputFileData%TwrCA, 'TwrCA', &
               'Normalized hydrodynamic added mass coefficient in Morison''s equation' )

            IF ( InputFileData%TwrCA < 0.0 )  CALL ProgAbort ( ' TwrCA must not be negative.' )


   ! TwrCD - Normalized hydrodynamic viscous drag coefficient in Morison's equation.

            CALL ReadVar ( UnIn, PtfmFile, InputFileData%TwrCD, 'TwrCD', &
               'Normalized hydrodynamic viscous drag coefficient in Morison''s equation' )

            IF ( InputFileData%TwrCD < 0.0 )  CALL ProgAbort ( ' TwrCD must not be negative.' )
         ...
         ELSE                       ! We must not be using the built-in Morison's equation, so skip these inputs.
            ...
            InputFileData%TwrCA   = 0.0
            InputFileData%TwrCD   = 0.0
            InputFileData%TwrDiam = 0.0
   ...
   ! Check to see if all WaveKinNd(:) analysis points are existing analysis points:
         ...
         DO I=1,NWaveKin
            IF ( ( WaveKinNd(I) < 1 ) .OR. ( WaveKinNd(I) > p%TwrNodes ) )  &
               CALL ProgAbort  ( ' All WaveKinNd values must be between 1 and '//TRIM( Num2LStr( p%TwrNodes ) )//' (inclusive).' )
         ENDDO ! I
...
!  -------------- MOORING LINES ------------------------------------------------
   ...
   !       instead of the radius, angle, and depth/draft.
            ...
            DO I = 1,NumLines ! Loop through all mooring lines
               ...
               CALL CheckIOS ( IOS, PtfmFile, 'mooring line '//TRIM( Num2LStr( I ) )//' properties', NumType )
               ...
               IF ( LRadAnch     <  0.0       )  &
                  CALL ProgAbort ( ' LRadAnch('//TRIM( Num2LStr( I ) )//') must not be less than zero.' )

               IF ( LRadFair     <  0.0       )  &
                  CALL ProgAbort ( ' LRadFair('//TRIM( Num2LStr( I ) )//') must not be less than zero.' )

               IF ( LDrftFair    <  p%TwrDraft  ) &
                  CALL ProgAbort ( ' LDrftFair('//TRIM( Num2LStr( I ) )//') must not be less than TwrDraft.' )

               IF ( LDpthAnch    <  LDrftFair ) &
                  CALL ProgAbort ( ' LDpthAnch('//TRIM( Num2LStr( I ) )//') must not be less than'// &
                               ' LDrftFair('//TRIM( Num2LStr( I ) )//').'                          )

               IF ( LUnstrLen(I) <= 0.0       )  &
                  CALL ProgAbort ( ' LUnstrLen('//TRIM( Num2LStr( I ) )//') must be greater than zero.' )

               IF ( LDiam    (I) <  0.0       )  &
                  CALL ProgAbort ( ' LDiam('//TRIM( Num2LStr( I ) )//') must not be less than zero.' )

               IF ( LMassDen (I) <  0.0       )  &
                  CALL ProgAbort ( ' LMassDen('//TRIM( Num2LStr( I ) )//') must not be less than zero.' )

               IF ( LEAStff  (I) <= 0.0       )  &
                  CALL ProgAbort ( ' LEAStff('//TRIM( Num2LStr( I ) )//') must be greater than zero.' )

               ! NOTE: Values of LSeabedCD less than zero indicate no seabed interaction (i.e., the mooring line may fall below the anchor)

               IF ( LTenTol  (I) <= 0.0       )  &
                  CALL ProgAbort ( ' LTenTol('//TRIM( Num2LStr( I ) )//') must be greater than zero.' )
      ...
      IF ( LineMod == 1 )  THEN  ! .TRUE if we have standard quasi-static mooring lines.
         DO I = 1,NumLines ! Loop through all mooring lines
            IF ( WtrDpth < -LAnchzi(I) )  & 
                  CALL ProgAbort ( ' WtrDpth must not be less than LDpthAnch('//TRIM( Num2LStr( I ) )//').' )
         ENDDO             ! I - All mooring lines
      ENDIF
   ...
   ! Check to see if all WaveKinNd(:) analysis points are existing analysis points:
      ...
      DO I=1,NWaveKin
         IF ( ( WaveKinNd(I) < 1 ) .OR. ( WaveKinNd(I) > PtfmNodes ) )  &
            CALL ProgAbort  ( ' All WaveKinNd values must be between 1 and '//TRIM( Num2LStr( PtfmNodes ) )//' (inclusive).' )
      ENDDO ! I
...
IF ( UsrTime /= 0.0 )  THEN
   ...
   CALL WrOver( ' Total Real Time:       '//TRIM( Num2LStr( Factor*ClckTime      ) )//TRIM( TimePer )// &
                '                                                 '                                   )
