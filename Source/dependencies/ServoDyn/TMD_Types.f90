...
! =========  TMD_InputFile  =======
  TYPE, PUBLIC :: TMD_InputFile
    ...
    INTEGER(IntKi)  :: TMD_CMODE      ! control mode (1:passive, 2: active) [-]
    ...
    REAL(ReKi)  :: TMD_X_M      ! TMD mass [-]
    REAL(ReKi)  :: TMD_Y_M      ! TMD mass [-]
    REAL(ReKi)  :: TMD_X_K      ! TMD stiffness [-]
    REAL(ReKi)  :: TMD_Y_K      ! TMD stiffness [-]
    REAL(ReKi)  :: TMD_X_C      ! TMD damping [-]
    REAL(ReKi)  :: TMD_Y_C      ! TMD damping [-]
    ...
    REAL(ReKi)  :: TMD_X_KS      ! stop spring stiffness [-]
    REAL(ReKi)  :: TMD_X_CS      ! stop spring damping [-]
    REAL(ReKi)  :: TMD_Y_PLSP      ! positive lateral stop position (maximum Y mass displacement) [-]
    REAL(ReKi)  :: TMD_Y_NLSP      ! negative lateral stop position (minimum Y mass displacement) [-]
    REAL(ReKi)  :: TMD_Y_KS      ! stop spring stiffness [-]
    REAL(ReKi)  :: TMD_Y_CS      ! stop spring damping [-]
    REAL(ReKi)  :: TMD_P_X      ! rest position of TMDs w.r.t. nacelle [-]
    REAL(ReKi)  :: TMD_P_Y      ! rest position of TMDs w.r.t. nacelle [-]
    REAL(ReKi)  :: TMD_P_Z      ! rest position of TMDs w.r.t. nacelle [-]
...
! =========  TMD_ParameterType  =======
  TYPE, PUBLIC :: TMD_ParameterType
    ...
    CHARACTER(1024)  :: RootName      ! RootName for writing output files [-]
    REAL(ReKi)  :: X_DSP      ! TMD_X initial displacement [-]
    REAL(ReKi)  :: Y_DSP      ! TMD_Y initial displacement [-]
    REAL(ReKi)  :: M_X      ! TMD mass [-]
    REAL(ReKi)  :: M_Y      ! TMD mass [-]
    REAL(ReKi)  :: K_X      ! TMD stiffness [-]
    REAL(ReKi)  :: K_Y      ! TMD stiffness [-]
    REAL(ReKi)  :: C_X      ! TMD damping [-]
    REAL(ReKi)  :: C_Y      ! TMD damping [-]
    REAL(ReKi) , DIMENSION(1:2)  :: K_S      ! TMD stop stiffness [-]
    REAL(ReKi) , DIMENSION(1:2)  :: C_S      ! TMD stop damping [-]
    REAL(ReKi) , DIMENSION(1:2)  :: P_SP      ! Positive stop position (maximum mass displacement) [-]
    REAL(ReKi) , DIMENSION(1:2)  :: N_SP      ! Negative stop position (minimum X mass displacement) [-]
